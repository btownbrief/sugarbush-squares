...
/* ------------------------------------------------------------- leaderboard */
// Monthly board for vs-bot wins only. Score = box margin, +1000 for Boiler
// wins so any Boiler win outranks any Sap Hauler win.
...
function botWinScore(status) {
  const margin = status.scores[RED] - status.scores[BLUE];
  return mode === 'boiler' ? 1000 + margin : margin;
}

// s >= 1000 means a Boiler win (margin = s - 1000); otherwise a Sap Hauler win
function lbScoreLabel(s) {
  return s >= 1000 ? `🔥 +${s - 1000} boxes` : `🪣 +${s} boxes`;
}
...
function finishGame(status, { celebrate = true, scoreFrom = null } = {}) {
  ...
  if (mode === 'hauler' || mode === 'boiler') {
    // Submit only on a fresh (celebrated) human win; losses and draws still
    // show the standings read-only. finishGame is re-called with
    // celebrate:false when repainting a finished board — never resubmit then.
    const humanWon = celebrate && !status.draw && status.winner === RED;
